...
(defun refbox-open-resource (target)
  "Open a propertized Refbox resource TARGET."
...
(defvar refbox-embark-resource-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "RET") #'refbox-open-resource)
    (define-key map (kbd "o") #'refbox-open-resource)
    map)
...
(defvar refbox-embark--target-injection-hooks
  '((refbox-insert-edit embark--ignore-target))
...
(defvar refbox-embark--default-action-overrides
  `((refbox-reference . refbox-run-default-action)
    ...
    (refbox-resource . refbox-open-resource))
...
(defun refbox-embark--citation-keys-and-bounds ()
  ...
  (cond
   ((and (derived-mode-p 'org-mode)
         (require 'refbox-org nil t))
    (pcase (refbox-org-citation-at-point)
      (`(,keys . (,begin . ,end))
       (list keys begin end))))
   ((and (derived-mode-p 'latex-mode 'LaTeX-mode 'tex-mode)
         (require 'refbox-latex nil t))
    (pcase (refbox-latex-citation-at-point)
      (`(,keys . (,begin . ,end))
       (list keys begin end))))
   ((and (derived-mode-p 'markdown-mode 'gfm-mode)
         (require 'refbox-markdown nil t))
    (pcase (refbox-markdown-citation-at-point)
      (`(,keys . (,begin . ,end))
       (list keys begin end))))))
